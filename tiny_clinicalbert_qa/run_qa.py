...
# jon-edward: Slightly modified to allow for local directory dataset usage and compatibility with emrQA-msquad.
...
class ModelArguments:
    ...
    finetuned_from: Optional[str] = field(
        default=None,
        metadata={"help": "Override 'finetuned_from' argument of the model."},
    )
...
class DataTrainingArguments:
    ...
    hub_dataset_names: Optional[List[str]] = field(
        default=None, metadata={"help": "The names of the datasets used which should appear on the hub model card."}
    )
...
def main():
    ...
    if training_args.do_train:
        column_names = list(set(raw_datasets["train"].column_names + ["id"]))
    elif training_args.do_eval:
        column_names = list(set(raw_datasets["validation"].column_names + ["id"]))
    else:
        column_names = list(set(raw_datasets["test"].column_names + ["id"]))
    question_column_name = "question" if "question" in column_names else column_names[0]
    context_column_name = "context" if "context" in column_names else column_names[1]
    answer_column_name = "answers" if "answers" in column_names else column_names[2]

    def process_dataset(dataset):
        # Add id column if there is none and remove answer_end in answer column
        if "id" not in dataset.column_names:
            dataset = dataset.add_column("id", [str(uuid.uuid4()) for _ in range(len(dataset))])
        col = []
        for answer in dataset[answer_column_name]:
            answer.pop("answer_end", None)
            col.append(answer)
        dataset = dataset.remove_columns(answer_column_name)
        dataset = dataset.add_column(answer_column_name, col)
        return dataset
    ...
    if training_args.do_eval:
        ...
        eval_examples = process_dataset(eval_examples)
